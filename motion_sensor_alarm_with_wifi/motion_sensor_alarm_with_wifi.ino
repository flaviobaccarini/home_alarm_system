...
String PATH_NAME   = "/trigger/alarm_on/with/key/cXtsDENM9GH34LAdr78CzSHUpjrfQDEzmjdXFcJryeq"; 
...
int sensorData = 0; // integer value for the status of the PIR sensor
...
void handle_waiting(unsigned long blink_time, 
                    ...
                    State next_STATE){
  /*
    Function for handling the passage of two states.
    In this function we make a LED blink and we wait
    some time before to pass in a new state.

    Parameters:
    ===========
    blink_time: unsigned long
                Delay time to wait to blink.

    pinLed: int
            Integer value for the LED that we want
            to blink.
    
    interval_to_wait: unsigned long
                      The time interval we have to wait
                      before to pass in a new state.
    
    next_STATE: State
                Next state in which the system goes after
                the interval_to_wait time.
  */
